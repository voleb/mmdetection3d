...
@INFERENCERS.register_module(name='det3d-mono-online')
@INFERENCERS.register_module()
class MonoDet3DInferencerOnline(Base3DInferencer):
    ...
    def __init__(self,
        ...
        super(MonoDet3DInferencerOnline, self).__init__(
            model=model,
            weights=weights,
            device=device,
            scope=scope,
    ...
    def _inputs_to_list(self,
        ...
        if isinstance(inputs, dict):
            ...
            for index, input in enumerate(inputs):
                ...
                cam2img = np.asarray(
                    data_info['images']['cam2img'], dtype=np.float32)
                lidar2cam = np.asarray(
                    data_info['images']['lidar2cam'],
                    dtype=np.float32)
